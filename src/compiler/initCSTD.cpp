...
void Compiler::addFunc(const Str& name, const Str& llvm_name, llvm::FunctionType* funcType, vector<std::tuple<Str, RecordStructType*, bool>>& params, RecordStructType* returnType, bool strictTypeCheck) {
...
void Compiler::addFunc2Mod(RecordModule* module, const Str& name, const Str& llvm_name, llvm::FunctionType* funcType, vector<std::tuple<Str, RecordStructType*, bool>>& params, RecordStructType* returnType, bool strictTypeCheck) {
    auto func = llvm::Function::Create(funcType, llvm::Function::ExternalLinkage, llvm_name, this->llvm_module.get());
...
void Compiler::_initilizeCSTDLib() {
    ...
    params = {std::make_tuple("useconds", this->gc_uint, false)};
    addFunc("usleep", "usleep", llvm::FunctionType::get(this->ll_int, {this->ll_uint}, false), params, this->gc_int, false);

    params = {std::make_tuple("dest", this->gc_raw_array, false), std::make_tuple("c", this->gc_int, false), std::make_tuple("n", this->gc_uint, false)};
