...
void Compiler::_initializeEnvironment() { env = std::make_shared<Enviornment>(std::make_shared<Enviornment>(nullptr, StrRecordMap(), "builtins"), StrRecordMap()); }

void Compiler::_initializeIRGCMap() {
    this->ir_gc_map_json = nlohmann::json{
        { "functions", json::object()},
        {   "structs", json::object()},
        {"GSinstance",  json::array()},
        {"GFinstance",  json::array()}
    };
}

void addBuiltinType(Compiler* compiler, const Str& name, llvm::Type* type) {
    auto record = std::make_shared<RecordStructType>(name, type);
    compiler->env->parent->addRecord(record);
}

void addBuiltinFunction(Compiler* compiler, const Str& name, llvm::FunctionType* funcType, const vector<std::tuple<Str, StructTypePtr, bool>>& params, StructTypePtr returnType) {
    auto func = llvm::Function::Create(funcType, llvm::Function::ExternalLinkage, name, compiler->llvm_module.get());
    compiler->env->parent->addRecord(std::make_shared<RecordFunction>(name, func, funcType, params, returnType, true));
}
...
void Compiler::_initializeBuiltins() {
    ...
    addBuiltinFunction(
        this,
        "puts",
        llvm::FunctionType::get(this->ll_void, this->ll_str, false),
        {
            {"Str", this->gc_str, false}
    },
        this->gc_void
    );
    addBuiltinFunction(
    ...
    addBuiltinFunction(
        ...
        {{"bits", this->gc_int, false}},
        this->gc_void
    );
    addBuiltinFunction(this, "free", llvm::FunctionType::get(this->ll_void, {this->ll_pointer}, true), {}, this->gc_void);
}
...
void Compiler::_decrementRC(llvm::Value* value) {
    // Pseudocode:
    // RC_ptr = load RC pointer from shared pointer structure
    ...
    this->llvm_ir_builder.CreateCall(
        this->env->getFunction("free", {this->env->getStruct("int")})->function,
        {llvm_ir_builder.CreateLoad(this->ll_pointer, this->llvm_ir_builder.CreateStructGEP(this->ll_shared_ptr, value, 0))}
    );
...
void Compiler::compile(shared_ptr<AST::Node> node) {
    ...
    switch (node->type()) {
        case AST::NodeType::Program:                      this->_visitProgram(node->castToProgram()); break;
        case AST::NodeType::ExpressionStatement:          this->_visitExpressionStatement(node->castToExpressionStatement()); break;
        case AST::NodeType::InfixedExpression:            this->_visitInfixExpression(node->castToInfixExpression()); break;
        case AST::NodeType::IndexExpression:              this->_visitIndexExpression(node->castToIndexExpression()); break;
        case AST::NodeType::VariableDeclarationStatement: this->_visitVariableDeclarationStatement(node->castToVariableDeclarationStatement()); break;
        case AST::NodeType::VariableAssignmentStatement:  this->_visitVariableAssignmentStatement(node->castToVariableAssignmentStatement()); break;
        case AST::NodeType::IfElseStatement:              this->_visitIfElseStatement(node->castToIfElseStatement()); break;
        case AST::NodeType::FunctionStatement:            this->_visitFunctionDeclarationStatement(node->castToFunctionStatement()); break;
        case AST::NodeType::CallExpression:               this->_visitCallExpression(node->castToCallExpression()); break;
        case AST::NodeType::ReturnStatement:              this->_visitReturnStatement(node->castToReturnStatement()); break;
        case AST::NodeType::RaiseStatement:               this->_visitRaiseStatement(node->castToRaiseStatement()); break;
        case AST::NodeType::TryCatchStatement:            this->_visitTryCatchStatement(node->castToTryCatchStatement()); break;
        case AST::NodeType::BlockStatement:               this->_visitBlockStatement(node->castToBlockStatement()); break;
        case AST::NodeType::WhileStatement:               this->_visitWhileStatement(node->castToWhileStatement()); break;
        case AST::NodeType::ForStatement:                 this->_visitForStatement(node->castToForStatement()); break;
        case AST::NodeType::BreakStatement:               this->_visitBreakStatement(node->castToBreakStatement()); break;
        case AST::NodeType::ContinueStatement:            this->_visitContinueStatement(node->castToContinueStatement()); break;
        case AST::NodeType::StructStatement:              this->_visitStructStatement(node->castToStructStatement()); break;
        case AST::NodeType::ImportStatement:              this->_visitImportStatement(node->castToImportStatement()); break;
        default:
...
void Compiler::_visitExpressionStatement(shared_ptr<AST::ExpressionStatement> expression_statement) { this->compile(expression_statement->expr); }
...
Compiler::ResolvedValue Compiler::_CallGfunc(
    const vector<GenericFunctionPtr>& gfuncs,
    const shared_ptr<AST::CallExpression> func_call,
    const Str& name,
    vector<llvm::Value*>& args,
    const vector<StructTypePtr>& params_types
) {
    ...
    for (const auto& gfunc : gfuncs) {
        ...
        for (size_t i = 0; i < params.size(); ++i) {
            ...
            llvm_param_types.push_back(
                param_type->struct_type || name == "array"
                    ? this->ll_shared_ptr
        ...
        auto llvm_return_type =
            return_type->struct_type || return_type->name == "array"
                ? this->ll_shared_ptr
                : return_type->stand_alone_type;
        ...
        auto func = llvm::Function::Create(
            ...
            this->fc_st_name_prefix != "main.gc.."
                ? this->fc_st_name_prefix + name
                : name,
        ...
        auto func_record = std::make_shared<RecordFunction>(
            name, func, func_type, std::vector<std::tuple<Str, StructTypePtr, bool>>(),
            return_type, gfunc->func->extra_info
        );
        ...
        if (body) {
            ...
            for (const auto& [idx, arg] : llvm::enumerate(func->args())) {
                ...
                llvm::Value* arg_value = param_references[idx]
                    ? this->llvm_ir_builder.CreateLoad(param_type->stand_alone_type, &arg, "loaded_" + arg.getName())
                    : alloca;
            ...
            func_record->set_meta_data(
                gfunc->func->meta_data.st_line_no,
                gfunc->func->meta_data.st_col_no,
                gfunc->func->meta_data.end_line_no,
                gfunc->func->meta_data.end_col_no
            );
            ...
            if (!this->function_entry_block.empty()) {
                this->llvm_ir_builder.SetInsertPoint(this->function_entry_block.back());
            }
        }
        else {
            // If no body, record the function arguments
            for (const auto& [idx, arg] : llvm::enumerate(func->args())) {
                func_record->arguments.emplace_back(
                    arg.getName().str(),
                    param_struct_types[idx],
                    param_references[idx]
                );
            }
    ...
    if (mismatches.empty()) {
        ...
        errors::NoOverload(
            ...
            "Check the function name or define the function."
        ).raise();
    }
    else {
        ...
        errors::NoOverload(
            ...
            func_call,
            "Argument types do not match any overload.",
            "Check the argument types or define an appropriate overload."
        ).raise();
    }
};

void Compiler::_createFunctionRecord(ASTFunctionStatementPtr function_declaration_statement,
                                     StructTypePtr struct_,
                                     std::shared_ptr<RecordModule> module,
    ...
    if (!function_declaration_statement->generic.empty()) {
        ...
        if (module) {
            ...
            errors::CompletionError("GenericInMethod", this->source,
                gsr->meta_data.st_line_no, gsr->meta_data.end_line_no,
                "Generics do not support struct members").raise();
        } else {
    ...
    for (const auto& param : params) {
        ...
        llvm::Type* llvm_param_type = param_type->stand_alone_type
                                       ? param_type->stand_alone_type
                                       : (param->value_type->refrence
                                          ? llvm::PointerType::getUnqual(param_type->stand_alone_type)
                                          : param_type->struct_type->getPointerTo());
    ...
    StructTypePtr return_type = function_declaration_statement->return_type
                                 ? this->_parseType(function_declaration_statement->return_type)
                                 : this->env->getStruct("void");
    llvm::Type* llvm_return_type = return_type->struct_type
                                    ? this->ll_shared_ptr
                                    : return_type->stand_alone_type;
    ...
    auto func_record = std::make_shared<RecordFunction>(name, func, func_type,
                                                         arguments, return_type,
                                                         function_declaration_statement->extra_info);
    ...
    if (auto body = function_declaration_statement->body) {
        ...
        for (auto& arg : func->args()) {
            ...
            if (is_reference) {
                arg_value = this->llvm_ir_builder.CreateLoad(
                    param_type_record->stand_alone_type
                    ? param_type_record->stand_alone_type
                    : param_type_record->struct_type, &arg);
            }
        ...
        func_record->set_meta_data(
            function_declaration_statement->meta_data.st_line_no,
...
Compiler::ResolvedValue Compiler::_callStruct(
    StructTypePtr struct_record,
    shared_ptr<AST::CallExpression> call_expression,
    vector<StructTypePtr> params_types,
    LLVMValueVector args
) {
    ...
    auto initializeNewStruct = [&](llvm::Value* allocaPtr) {
        ...
        auto value_gep = this->llvm_ir_builder.CreateGEP(
            this->ll_shared_ptr, allocaPtr, llConstInt::get(this->ll_int, 0)
        );
        ...
        llvm::Value* gep = this->llvm_ir_builder.CreateGEP(
            ...
            llConstInt::get(llvm::Type::getInt64Ty(this->llvm_context), 1)
        );
        llvm::Value* size = this->llvm_ir_builder.CreatePtrToInt(
            gep, llvm::Type::getInt64Ty(this->llvm_context)
        );

        // Allocate memory for the struct
        auto struct_alloca = this->llvm_ir_builder.CreateCall(
            this->env->getFunction("malloc", {this->env->getStruct("int")})->function,
            {size}
        );
        this->llvm_ir_builder.CreateStore(struct_alloca, value_gep);

        // Allocate and initialize reference count
        auto RC_alloca = this->llvm_ir_builder.CreateCall(
            this->env->getFunction("malloc", {this->env->getStruct("int")})->function,
            {llConstInt::get(this->ll_int, this->ll_int->getScalarSizeInBits())}
        );
        auto RC_gep = this->llvm_ir_builder.CreateGEP(
            this->ll_shared_ptr, allocaPtr, llConstInt::get(this->ll_int, 1)
        );
        this->llvm_ir_builder.CreateStore(llConstInt::get(this->ll_int, 0), RC_alloca);
    ...
    auto initializeExistingStruct = [&](llvm::Value* allocaPtr) {
        ...
        this->llvm_ir_builder.CreateStore(
            llvm::ConstantPointerNull::get(this->ll_pointer), RC_gep
        );
    };
...
void Compiler::_createClearMethod(StructTypePtr struct_record) {
    ...
    auto clear_func = llvm::Function::Create(
        clear_func_type,
        llvm::Function::ExternalLinkage,
        func_name,
        this->llvm_module.get()
    ...
    for (const auto& field_name : fields) {
        ...
        llvm::Value* field = this->llvm_ir_builder.CreateLoad(
            this->ll_shared_ptr,
            gep,
            field_name.c_str()
        );
    ...
    struct_record->gc_struct_clear = std::make_shared<RecordFunction>(
        "__clear__",
        clear_func,
        clear_func_type,
        std::vector<std::tuple<Str, StructTypePtr, bool>>(),
        this->env->getStruct("void")
    );
}

Compiler::ResolvedValue Compiler::_CallGstruct(
    const vector<GenericStructTypePtr>& gstructs,
    const shared_ptr<AST::CallExpression> func_call,
    const Str& name,
    vector<llvm::Value*>& args,
    const vector<StructTypePtr>& params_types
        ...
        if (!gstruct->env->isStruct(struct_name, false, generics)) {
            ...
            for (const auto& field : fields) {
                if (field->type() == AST::NodeType::VariableDeclarationStatement) {
                    ...
                    llvm::Type* llvm_field_type = (field_type->stand_alone_type || field_type->name == "array")
                        ? this->ll_shared_ptr
                        : field_type->stand_alone_type;
                    ...
                    auto struct_type = llvm::StructType::create(
                        this->llvm_context, field_types, this->fc_st_name_prefix + struct_name
                    );
...
void Compiler::_createStructRecord(
    ...
    if (!struct_statement->generics.empty()) {
        ...
        this->env->addRecord(gsr);
        if (module) {
            module->record_map.emplace_back(struct_name, gsr);
        }
        return;
    }
...
void Compiler::_processFieldDeclaration(
    AST::NodePtr field,
    ASTStructStatementPtr struct_statement,
    ...
    llvm::Type* llvm_field_type = (field_type->struct_type || field_type->name == "array")
        ? this->ll_shared_ptr
        : field_type->stand_alone_type;
...
void Compiler::_handleGenericSubType(
    AST::NodePtr field,
    ASTStructStatementPtr struct_statement,
    std::shared_ptr<RecordStructType> struct_record,
...
void Compiler::_processFieldFunction(
    AST::NodePtr field,
    std::shared_ptr<RecordStructType> struct_record,
    const nlohmann::json& ir_gc_map_json
) {
...
void Compiler::_visitStructStatement(ASTStructStatementPtr struct_statement) { this->_createStructRecord(struct_statement, nullptr, this->ir_gc_map_json); }

Compiler::ResolvedValue Compiler::convertType(const std::tuple<llvm::Value*, llvm::Value*, StructTypePtr>& from, StructTypePtr to) {
    auto [fromloadedval, fromalloca, fromtype] = from;
    ...
    if (fromtype->name == "int32" && to->name == "int") {
        ...
        if (fromtype->is_method(
            ...
            auto method = fromtype->get_method(
                "",
                {
                    fromtype
            },
...
bool Compiler::conversionPrecidence(StructTypePtr from, StructTypePtr to) {
    const vector<std::pair<Str, Str>> precedencePairs = {
        {  "int32",     "int"},
        { "uint32",     "int"},
        {   "uint",     "int"},
        { "uint32",    "uint"},
    ...
    if (right->type() == AST::NodeType::IdentifierLiteral) {
        ...
        if (ltt == resolveType::Module) {
            ...
            if (left_type->isGenericFunc(name) ? left_type->isFunction(name, params_types, true) : left_type->isFunction(name, params_types)) {
                ...
                if (call_expression->_new) {
                    llvm::Value* gep =
                        this->llvm_ir_builder
                            .CreateGEP(struct_type, llvm::ConstantPointerNull::get(struct_type->getPointerTo()), llConstInt::get(llvm::Type::getInt64Ty(this->llvm_context), 1), "gep" + name);
                    llvm::Value* size = this->llvm_ir_builder.CreatePtrToInt(gep, this->ll_int, name + "_size");
                    alloca            = this->llvm_ir_builder.CreateCall(this->env->getFunction("malloc", {this->env->getStruct("int")})->function, {size}, name + "_heap_pointer");
                } else alloca = this->llvm_ir_builder.CreateAlloca(struct_type, nullptr, name);
                params_types.insert(params_types.begin(), struct_record);
...
Compiler::ResolvedValue Compiler::_visitIndexExpression(shared_ptr<AST::IndexExpression> index_expression) {
    ...
    if (_checkType(left_generic, this->env->getStruct("array"))) {
...
void Compiler::_visitVariableDeclarationStatement(shared_ptr<AST::VariableDeclarationStatement> variable_declaration_statement) {
    ...
    if (var_value == nullptr) {
        ...
        if (vartt != resolveType::StructInst) {
        ...
        if (!_checkType(var_generic, var_type)) {
            if (this->canConvertType(var_generic, var_type)) {
                ...
                var_generic        = std::get<StructTypePtr>(x.variant);
            } else {
...
vector<Str> splitStr(const Str& input) {
    vector<Str> result;
    size_t start = 0;
...
void Compiler::_visitVariableAssignmentStatement(shared_ptr<AST::VariableAssignmentStatement> variable_assignment_statement) {
    ...
    if (att != resolveType::StructInst) {
        errors::WrongType(this->source, variable_assignment_statement->name, {std::get<StructTypePtr>(_var_type)}, "Cannot assign to ltype").raise();
        exit(1);
    }
...
Compiler::ResolvedValue Compiler::_resolveValue(shared_ptr<AST::Node> node) {
    switch (node->type()) {
        case AST::NodeType::IntegerLiteral: {
            auto integer_literal = node->castToIntegerLiteral();
            auto value           = llConstInt::get(this->llvm_context, llvm::APInt(64, integer_literal->value));
        ...
        case AST::NodeType::FloatLiteral: {
            auto float_literal = node->castToFloatLiteral();
        ...
        case AST::NodeType::StringLiteral: {
            auto Str_literal = node->castToStringLiteral();
            auto value       = this->llvm_ir_builder.CreateGlobalStringPtr(Str_literal->value);
            return {value, value, this->env->getStruct("str"), resolveType::StructInst};
        }
        case AST::NodeType::IdentifierLiteral: {
            auto identifier_literal = node->castToIdentifierLiteral();
        ...
        case AST::NodeType::InfixedExpression: {
            return this->_visitInfixExpression(node->castToInfixExpression());
        }
        case AST::NodeType::IndexExpression: {
            return this->_visitIndexExpression(node->castToIndexExpression());
        }
        case AST::NodeType::CallExpression: {
            return this->_visitCallExpression(node->castToCallExpression());
        }
        case AST::NodeType::BooleanLiteral: {
            auto boolean_literal = node->castToBooleanLiteral();
            auto value           = boolean_literal->value ? llConstInt::getTrue(this->llvm_context) : llConstInt::getFalse(this->llvm_context);
        ...
        case AST::NodeType::ArrayLiteral: {
            return this->_visitArrayLiteral(node->castToArrayLiteral());
        }
        default: {
            errors::CompletionError("Unknown node type", this->source, node->meta_data.st_line_no, node->meta_data.end_line_no, "Unknown node type: " + AST::nodeTypeToString(node->type())).raise();
            exit(1);
...
Compiler::ResolvedValue Compiler::_visitArrayLiteral(shared_ptr<AST::ArrayLiteral> array_literal) {
    vector<llvm::Value*> values;
    StructTypePtr struct_type = nullptr;
    StructTypePtr first_generic;
    ...
    for (auto element : array_literal->elements) {
        ...
        auto generic = std::get<StructTypePtr>(_generic);
        if (!struct_type) {
            struct_type   = generic;
        ...
        if (!_checkType(first_generic, generic)) {
            if (this->canConvertType(generic, first_generic)) {
                ...
                generic      = std::get<StructTypePtr>(x.variant);
            } else {
    ...
    } else this->llvm_ir_builder.CreateStore(llvm::ConstantPointerNull::get(this->ll_pointer), this->llvm_ir_builder.CreateStructGEP(this->ll_shared_ptr, shared_array, 1));
    return {shared_array, shared_array, array_struct, resolveType::StructInst};
};

void Compiler::_visitReturnStatement(shared_ptr<AST::ReturnStatement> return_statement) {
    auto value = return_statement->value;
    if (!value && this->env->current_function->return_type->name == "void") {
        this->llvm_ir_builder.CreateRetVoid();
        throw DoneRet(); // Throws `DoneRet` Error to Indicate that Dont parse the following statements in the current block statement
    }
    ...
    if (rtt != resolveType::StructInst) {
        if (rtt == resolveType::StructType && std::get<StructTypePtr>(_)->name == "void") {
            this->llvm_ir_builder.CreateRetVoid();
            throw DoneRet(); // Throws `DoneRet` Error to Indicate that Dont parse the following statements in the current block statement
        } else {
    ...
    auto return_type = std::get<StructTypePtr>(_);
    if (!_checkType(this->env->current_function->return_type, return_type)) {
        if (this->canConvertType(return_type, this->env->current_function->return_type)) {
            ...
            if (!return_value) {
                errors::WrongType(this->source, value, {this->env->current_function->return_type}, "Return Type mismatch").raise();
                exit(1);
            }
        } else {
            errors::WrongType(this->source, value, {this->env->current_function->return_type}, "Return Type mismatch").raise();
    ...
    if (return_type->struct_type) this->_incrementRC(return_value);
    for (auto var : this->env->current_function->env->getCurrentVars()) {
        if (var->variable_type->struct_type || var->variable_type->name == "array") { this->_decrementRC(var->value); }
    }
...
Compiler::ResolvedValue Compiler::_visitCallExpression(shared_ptr<AST::CallExpression> call_expression) {
    ...
    for (auto arg : param) {
        ...
        if (ptt != resolveType::StructInst && ptt != resolveType::StructType) {
...
void Compiler::_visitIfElseStatement(shared_ptr<AST::IfElseStatement> if_statement) {
    ...
    if (ctt != resolveType::StructInst || !_checkType(std::get<StructTypePtr>(_condition), this->env->getStruct("bool"))) {
        errors::WrongType(this->source, if_statement->condition, {this->env->getStruct("bool")}, "if else Condition must be bool").raise();
        exit(1);
    }

    auto func    = this->llvm_ir_builder.GetInsertBlock()->getParent();
    llBB* ThenBB = llBB::Create(this->llvm_context, "then", func);
    llBB* ContBB = llBB::Create(this->llvm_context, "cont", func);
    llBB* ElseBB = if_statement->alternative ? llBB::Create(this->llvm_context, "else", func) : nullptr;
...
void Compiler::_visitWhileStatement(shared_ptr<AST::WhileStatement> while_statement) {
    ...
    llBB* CondBB = llBB::Create(this->llvm_context, "cond", func);
    llBB* BodyBB = llBB::Create(this->llvm_context, "body", func);
    llBB* ContBB = llBB::Create(this->llvm_context, "cont", func);
    ...
    if (ctt != resolveType::StructInst) {
        errors::WrongType(this->source, condition, {this->env->getStruct("bool")}, "while loop Condition can't be module or type").raise();
        exit(1);
    }

    auto bool_condition = std::get<StructTypePtr>(_condition);
    if (!_checkType(bool_condition, this->env->getStruct("bool"))) {
        if (this->canConvertType(bool_condition, this->env->getStruct("bool"))) {
            std::cout << "SOB" << std::endl;
            auto x        = this->convertType({condition_val, _, bool_condition}, this->env->getStruct("bool"));
            condition_val = x.value;
        } else {
            errors::WrongType(this->source, condition, {this->env->getStruct("bool")}, "while loop Condition must be bool").raise();
            exit(1);
        }
    }


    this->env->loop_body_block.push_back(BodyBB);
    this->env->loop_conti_block.push_back(ContBB);
    this->env->loop_condition_block.push_back(CondBB);
    llBB* IfBreakBB  = nullptr;
    llBB* NotBreakBB = nullptr;
    if (while_statement->ifbreak) {
        IfBreakBB = llBB::Create(this->llvm_context, "ifbreak", func);
        this->env->loop_ifbreak_block.push_back(IfBreakBB);
    } else this->env->loop_ifbreak_block.push_back(nullptr);
    if (while_statement->notbreak) {
        NotBreakBB = llBB::Create(this->llvm_context, "notbreak", func);
        this->env->loop_notbreak_block.push_back(NotBreakBB);
        this->llvm_ir_builder.CreateCondBr(condition_val, BodyBB, NotBreakBB);
    } else {
        this->env->loop_notbreak_block.push_back(nullptr);
    ...
    this->env->loop_body_block.pop_back();
    this->env->loop_conti_block.pop_back();
    this->env->loop_condition_block.pop_back();
    this->env->loop_ifbreak_block.pop_back();
    this->env->loop_notbreak_block.pop_back();
}
...
void Compiler::_visitForStatement(shared_ptr<AST::ForStatement> for_statement) {
    ...
    if (w != resolveType::StructInst) {
        errors::WrongType(this->source, for_statement->from, {}, "cant loop over module or type").raise();
        exit(1);
    }

    auto loop_from_type = std::get<StructTypePtr>(_loop_from_type);
    if (loop_from_type->is_method("__iter__", {loop_from_type})) {
        ...
        if (!(loop_from_type->is_method("__next__", {loop_from_type, iter_type}) && loop_from_type->is_method("__done__", {loop_from_type, iter_type}, {}, this->env->getStruct("bool")))) {
        ...
        if (iter_type->stand_alone_type) this->llvm_ir_builder.CreateStore(this->llvm_ir_builder.CreateCall(iter_geter->function, {loop_from_alloca}), iterator_alloca);
        else iterator_alloca = this->llvm_ir_builder.CreateCall(iter_geter->function, {loop_from_alloca});
        llBB* IfBreakBB  = nullptr;
        llBB* NotBreakBB = nullptr;
        ...
        if (next_method->return_type->struct_type) {
            this->llvm_ir_builder.CreateStore(
                this->llvm_ir_builder.CreateLoad(next_method->return_type->struct_type, this->llvm_ir_builder.CreateCall(next_method->function, {loop_from_alloca, iterator_alloca})),
                alloca
            );
        } else {
        ...
        VariablePtr var;
        if (next_method->return_type->struct_type) var = std::make_shared<RecordVariable>(for_statement->get->value, alloca, alloca, next_method->return_type);
        else var = std::make_shared<RecordVariable>(for_statement->get->value, this->llvm_ir_builder.CreateLoad(next_method->return_type->stand_alone_type, alloca), alloca, next_method->return_type);
        this->env->addRecord(var);
...
void Compiler::addFieldToStruct(StructTypePtr struct_record, shared_ptr<AST::VariableDeclarationStatement> field_decl, vector<llvm::Type*>& field_types, const Str& struct_name) {
    Str field_name = field_decl->name->castToIdentifierLiteral()->value;

    auto field_type = this->_parseType(field_decl->value_type);
    field_types.push_back(field_type->struct_type || field_type->name == "array" ? this->ll_shared_ptr : field_type->stand_alone_type);
    struct_record->addSubType(field_name, field_type);
...
void Compiler::_visitImportStatement(shared_ptr<AST::ImportStatement> import_statement, shared_ptr<RecordModule> module) {
    ...
    // Determine the path for the imported source
    ...
    // Load IR-GC map
    json ir_gc_map_json;
    std::filesystem::path ir_gc_map = this->ir_gc_map.parent_path() / (relative_path + ".gc.json");
    if (relative_path.starts_with("std/")) { ir_gc_map = GC_STD_IRGCMAP / (relative_path.substr(4) + ".gc.json"); }
    ...
    if (!ir_gc_map_json["uptodate"]) {
        std::cerr << "IR GC map not up-to-date for: " << gc_source_path << std::endl;
        throw NotCompiledError(gc_source_path.string());
    }

    // Read the source code
    Str gc_source   = readFileToString(gc_source_path.string());
    Str prev_source = this->source;
    ...
    // Setup environment for import
    auto prev_env = this->env;
    this->env     = std::make_shared<Enviornment>(prev_env, StrRecordMap{}, module_name);

    // Create a new module record if importing into an existing module
    ...
    // Iterate over statements and utilize helper functions
    for (auto& stmt : program->statements) {
        switch (stmt->type()) {
            case AST::NodeType::FunctionStatement: this->_createFunctionRecord(stmt->castToFunctionStatement(), nullptr, import_module, ir_gc_map_json); break;
            case AST::NodeType::StructStatement:   this->_createStructRecord(stmt->castToStructStatement(), import_module, ir_gc_map_json); break;
            case AST::NodeType::ImportStatement:   this->_visitImportStatement(stmt->castToImportStatement(), import_module); break;
            default:                               std::cerr << "Unknown statement type during import, skipping." << std::endl; break;
        }
    }

    // Restore previous state
    this->file_path = this->file_path.parent_path(); // Adjust as necessary
    this->env       = prev_env;
    this->source    = prev_source;
}

void Compiler::_importFunctionDeclarationStatement(ASTFunctionStatementPtr function_declaration_statement, std::shared_ptr<RecordModule> module, nlohmann::json& ir_gc_map_json) {
...
void Compiler::_importStructStatement(ASTStructStatementPtr struct_statement, std::shared_ptr<RecordModule> module, nlohmann::json& ir_gc_map_json) {
    // Utilize the helper function with the module and ir_gc_map_json
