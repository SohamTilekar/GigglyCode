...
void compileFile(const std::string& filePath, const std::string& outputFilePath, const std::string& ir_gc_map, const std::string& objFilePath, const std::string& buildDir, const std::string& relativePath, json& compiledFilesRecord, const std::string& optimizationLevel) {
    std::string fileContent = readFileToString(filePath);
            ...
            if (entry.path().extension() == ".c") {
                ...
                while (!filesRecord.empty()) {
                    try {
                        ...
                        compileFile(std::get<0>(fileTuple), std::get<1>(fileTuple), std::get<2>(fileTuple), std::get<3>(fileTuple), buildDir, std::get<4>(fileTuple), compiledFilesRecord, optimizationLevel);
...
int main(int argc, char* argv[]) {
    ...
    std::string linkCommand = "clang -v -O3 " + objFiles + "-o " + executablePath;
